---
name: c-code
description: 'Manage local code inventory (`data/c-code-repos.json`). Windows (`C:\Users\gotmo\Code`) is primary for full workflows (scripts, sync). Termux/proot-distro Ubuntu env is READ-ONLY: read files and direct `git clone` allowed, but DO NOT run bundled scripts. macOS/Linux require `_meta.root_path` edits. Triggers: "Code文件夹", local repos, forks, cloning (esp. from `stars-readme/`), or updating inventory. Uses `gh repo clone` with conflict detection for canonical folder names.'
---
