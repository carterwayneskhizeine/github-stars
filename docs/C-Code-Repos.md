# `C:\Users\gotmo\Code` 仓库清单 — 使用说明书

> **本文件是 `data/c-code-repos.json` 的用户手册**。
...
| 你想… | 怎么做 |
| --- | --- |
| 看 `C:\Users\gotmo\Code` 里有什么 | `cat data/c-code-repos.json` 或用 `jq`（见下方查询示例） |
| 知道哪些是 fork 哪些是 clone | `jq '.repos[] \| select(.type=="fork")' data/c-code-repos.json` |
| 把 star 里的 X 克隆到 `Code` 文件夹 | **直接跟 agent 说「克隆 https://github.com/X/Y」** — skill 会自动跑「克隆 → 同步 stars → 回填 `source_star`」整条链（详见「运行环境」章节）|
| 重新扫一遍本地 | `python .agents/skills/c-code/scripts/scan_inventory.py` |
| 跨平台（macOS / Linux / Termux） | macOS / Linux：改 JSON 的 `_meta.root_path` 后重跑 scan。Termux 下不能写，详见「运行环境」和 §6 |
| 找 star 了但还没 clone 的 | `python .agents/skills/c-code/scripts/clone_repo.py --list-new` |
...
github-stars/
├── data/
│   └── c-code-repos.json    ← 唯一数据源（_meta + repos[]）
...
> **改 JSON 之前先读一下 [`references/json-schema.md`](../.agents/skills/c-code/references/json-schema.md)**——那里有完整的字段定义和示例。
...
完整字段语义见 [`references/json-schema.md`](../.agents/skills/c-code/references/json-schema.md)。

---

## 运行环境（三种 mode）

c-code skill 在跑任何写文件的脚本（`clone_repo.py` / `scan_inventory.py` / `download_stars.py`）之前，会先检测自己在哪种环境里。三种 mode 对应不同的允许动作，**不满足条件就拒绝执行并问用户**，不会吞错或瞎猜。
...
|---|---|---|---|
| `windows-local` | Windows + `C:\Users\gotmo\Code` 存在 | clone / scan / backfill / query / 任何状态写入 | — |
...
| `other` | 其他（macOS / Linux 无 Termux / 其他 Windows 路径 / …）| 等待用户说明 | 不能猜，会问「你想怎么走？」|

### Termux 模式该怎么办

如果你在手机上用 Termux 的 Ubuntu 模拟器（proot-distro）跑这个 skill，**`C:\Users\gotmo\Code` 没有对应物**，而且大概率没装 `gh`。所以 c-code skill 不会真去克隆——它会告诉你：
...
2. `clone_repo.py https://github.com/Einsia/Browser-BC` → `C:\Users\gotmo\Code\Browser-BC\`
3. `python download_stars.py` → 同步 stars，`stars-readme/Einsia-Browser-BC.md` 落盘
4. `scan_inventory.py --backfill-source-star` → `data/c-code-repos.json` 里 `Browser-BC.source_star = "Einsia-Browser-BC.md"`
...
```bash
# 总览
jq '.repos | length' data/c-code-repos.json           # 总数
jq '._meta.last_full_scan' data/c-code-repos.json     # 上次扫描时间

# 按类型筛选
jq '.repos[] | select(.type=="fork") | .name' data/c-code-repos.json
jq '.repos[] | select(.type=="upstream-clone") | .name' data/c-code-repos.json

# Fork 详情（看是从哪个父仓 fork 的）
jq '.repos[] | select(.type=="fork") | "\(.name) → \(.parent)"' data/c-code-repos.json

# 找还没从 star 克隆下来的（交叉 data/stars_mapping.json）
jq -r '.repos[] | select(.source_star==null) | .name' data/c-code-repos.json
```
...
```python
import json
with open("data/c-code-repos.json", encoding="utf-8") as f:
    inv = json.load(f)
...
data/stars_mapping.json       ←  Owner-Repo.md → GitHub URL 映射
           ↓ 怎么用
data/c-code-repos.json        ←  本地 clone 状态（scan_inventory.py 产出）  ★ 唯一数据源
           ↓ 怎么用
docs/C-Code-Repos.md          ←  本文件
```

找 **star 了但还没 clone** 的 repo：

```bash
python .agents/skills/c-code/scripts/clone_repo.py --list-new
```
...
4. 写回 `data/c-code-repos.json`

加 `--diff` 只看变化不写：
```bash
python .agents/skills/c-code/scripts/scan_inventory.py --diff
```
...
很多 upstream-clone 是在 c-code skill 出现**之前**手动 clone 的，JSON 里它们的 `source_star` 是 `null`，没法判断是"老 clone 找不到 star 来源"还是"真没 star 过"。
...
在 `windows-local` 模式下，c-code skill 跑的是「**clone → sync stars → backfill source_star**」三合一的完整工作流。你只需要跟 agent 说一句「把 https://github.com/X/Y 克隆到 Code 文件夹」，skill 会自动：
...
```bash
# Step 1: 克隆（带冲突检测）
python .agents/skills/c-code/scripts/clone_repo.py vercel/eve
python .agents/skills/c-code/scripts/clone_repo.py https://github.com/a2ui-project/a2ui
python .agents/skills/c-code/scripts/clone_repo.py stars-readme/3DTopia-MVPaint.md
...
成功后自动更新 `data/c-code-repos.json`。

加 `--yes` 跳过交互（冲突时默认用"改名字"）：
```bash
python .agents/skills/c-code/scripts/clone_repo.py --yes <target>
```
...
3. 编辑 `data/c-code-repos.json` 的 `_meta.root_path`：
   - Windows: `"C:\\Users\\gotmo\\Code"`
   - macOS: `"/Users/<you>/Code"`
   - Linux: `"/home/<you>/Code"`
4. 跑 `python .agents/skills/c-code/scripts/scan_inventory.py` 重新生成
...
如果你在手机上跑 Termux + proot-distro Ubuntu，**不要试图让 c-code skill 在这里 clone / scan / backfill**：

- `C:\Users\gotmo\Code` 没有 Linux 对应物
- 就算改成 `/data/data/com.termux/files/home/Code` 也带不走（没持久化、没 `gh`、没授权）
- skill 会自动检测到 `termux-ubuntu` mode 并拒绝执行

能在这个环境里做的事：

- ✅ **读**：`data/c-code-repos.json` 查询、`stars-readme/*.md` 内容浏览
